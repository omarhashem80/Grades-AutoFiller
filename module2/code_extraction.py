...
def segment_id(code):
    ...
    bounding_rectangles = [
        rect for rect in bounding_rectangles if rect[2] * rect[3] > 100
    ]
...
def extract_student_code(paper):
    ...
    contours, _ = cv2.findContours(
        threshold_binary_image, cv2.RETR_TREE, cv2.CHAIN_APPROX_NONE
    )
...
def detect_marked_answers(
    sorted_contours, eroded_image, number_of_questions, circle_width
):
    """
...
def get_student_bubble_code(paper):
    ...
    questions_per_row, answers_per_question = calculate_row_and_columns(
        circles_contours, circle_width
    )
    number_of_questions = len(circles_contours) // answers_per_question

    student_answers, chosen_contours = detect_marked_answers(
        sorted_contours, eroded_image, number_of_questions, circle_width
    )
